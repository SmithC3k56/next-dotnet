...
//exporte client
export const client = new JsonServiceClient(process.env.apiBaseUrl).apply(
  (c) => {
    c.basePath = "/api";
...
export function useApp() {
  ...
  const getType = (name: string) =>
    appState.metadata?.types?.find(
      (x) => x.name?.toLowerCase() == name.toLocaleLowerCase()
    );
  ...
  const enumOptions = (name: string) => {
    ...
    if (type && type.isEnum && type.enumNames != null) {
      for (let i = 0; i < type.enumNames.length; i++) {
        ...
        const key =
          (type.enumValues != null ? type.enumValues[i] : null) ?? name;
        to[key] = name;
      }
    }
    return to;
...
export type ClientContext = ApiState & {
  ...
  apiVoid(
    ...
    args?: any,
    method?: string
  ): Promise<ApiResult<EmptyResponse>>;
};
